...
The next stage in TAVI guidance is the transition toward robot-assisted and potentially autonomous delivery platforms. Early clinical and preclinical robotic systems are already emerging, including the TAVIPILOT platform from Caranx Medical \cite{smits2025_autonomous,caranx2025_tavipilot,clinicaltrials_nct07152574}. Such systems require not only a contour of the aortic root, but also reliable and temporally stable localization of key landmarks, especially the two annular points (AA1 and AA2) and the two sinotubular-junction landmarks near the coronary ostia (STJ1 and STJ2).
...
Training was performed in PyTorch with PyTorch Lightning. AdamW was used as the optimizer with an initial learning rate of $1 \times 10^{-3}$ and weight decay of $3 \times 10^{-4}$. The learning rate schedule followed CosineAnnealingLR. To stabilize optimization, several parameters were scheduled smoothly. The soft-argmax temperature increased during early epochs, enabling a transition from smoother to more localized coordinate distributions. At the same time, the width of the Gaussian target heatmaps was gradually reduced so that training started from softer targets and then shifted toward higher spatial precision.
...
The landmark branch receives fused features from the deepest encoder stage and the segmentation pathway. It consists of local convolutional processing, a Coordinate Attention module that preserves vertical and horizontal positional structure, and three output components:
\begin{enumerate}
\item landmark heatmaps;
\item local offset maps used to refine coordinates;
\item a boundary-weight map that highlights probable anatomical boundaries.
\end{enumerate}

This design allows the model to exploit both global root morphology and local contour geometry when localizing landmarks. The architecture diagram present in the markdown draft was intentionally omitted from the main manuscript at this stage because of figure-count constraints.

\subsection{Landmark coordinate extraction}\label{subsec:coordinates}

Landmark coordinates were estimated in two stages. First, coarse coordinates were obtained from predicted heatmaps using differentiable soft-argmax, which preserved gradient flow and enabled end-to-end training. For the $k$-th landmark, the pixel probability at location $(i,j)$ was defined as
\begin{equation}
P_{ij}^{(k)}=
\frac{\exp\left(\beta H_{ij}^{(k)}\right)}
{\sum\limits_{m,n}\exp\left(\beta H_{mn}^{(k)}\right)},
\end{equation}
where $H_{ij}^{(k)}$ is the heatmap logit and $\beta$ is the temperature parameter. The initial coordinates were then computed as expectations:
\begin{equation}
\hat{x}_k^{hm}=\sum\limits_{i,j} j\,P_{ij}^{(k)}, \qquad
\hat{y}_k^{hm}=\sum\limits_{i,j} i\,P_{ij}^{(k)}.
\end{equation}

In the second stage, these coordinates were refined with dedicated offset maps that predicted local horizontal and vertical corrections:
\begin{equation}
\hat{x}_k=\hat{x}_k^{hm}+\Delta x_k, \qquad
\hat{y}_k=\hat{y}_k^{hm}+\Delta y_k.
\end{equation}

This formulation combines the robustness of heatmap-based localization with sub-pixel coordinate refinement. In addition, the predicted boundary map was used inside the landmark branch to softly amplify features in anatomically informative regions.
...
Segmentation was optimized with a combination of focal loss and soft Dice loss, which jointly addressed class imbalance and overlap quality. Landmark supervision consisted of heatmap matching together with a coordinate regression term:
\begin{equation}
\mathcal{L}_{coord} =
\frac{1}{\sum_k v_k}
\sum_{k=1}^{4}
...
The overall performance of BAMNet appears to arise not from a single isolated component but from the interaction of multiple design choices. The architecture combines an EfficientNet-V2 encoder, a modified MA-Net decoder, global spatial attention, a dedicated landmark head, boundary-aware supervision, and coordinate-sensitive feature modulation. Together, these components make it possible to maintain strong segmentation quality while also delivering clinically meaningful landmark coordinates.

Ablation experiments reported in the source draft support the importance of these components. The full model achieved the best Dice score (0.8846) and one of the strongest composite balances between Dice and point error (0.7180). Removing PositionAttention reduced Dice to 0.8720 and increased point error to 12.08~px, indicating the usefulness of global spatial context under low-contrast and partially occluded fluoroscopic conditions. Excluding Coordinate Attention similarly degraded performance (Dice~=~0.8719, point error~=~12.11~px), which is consistent with the role of coordinate-aware enhancement for anatomical landmark localization.

Feature fusion and offset refinement were also important. Removing fusion reduced Dice to 0.8696, while removing offsets yielded Dice~=~0.8757 with a point error of 12.13~px, again worsening the overall balance. Boundary-guided supervision showed a more nuanced effect: removing boundary guidance reduced Dice to 0.8738 and increased point error to 11.46~px, whereas removing the explicit boundary loss reduced Dice to 0.8789 but slightly improved point error to 11.42~px. This pattern suggests that boundary supervision primarily improves anatomical consistency of the mask and contour regularity, while gains in coordinate accuracy may depend on checkpoint selection criteria and the overall training balance.

The soft-argmax temperature schedule was another meaningful factor. In the draft ablation study, the $\beta$ schedule from 4 to 8 was inferior to a fixed $\beta=8$ setup, which indicates that the localization-segmentation trade-off is sensitive to how sharply heatmaps are converted to coordinates. From a deployment perspective, the main practical benefit is that BAMNet removes the need to run two separate models at inference time. The reported throughput of 43.9~FPS on an NVIDIA RTX4070 is compatible with real-time fluoroscopic assistance.
...
\noindent\textbf{Funding} TODO.

\noindent\textbf{Conflict of interest/Competing interests} TODO.

\noindent\textbf{Ethics approval and consent to participate} TODO.

\noindent\textbf{Consent for publication} TODO.

\noindent\textbf{Data availability} TODO.

\noindent\textbf{Materials availability} TODO.

\noindent\textbf{Code availability} TODO.

\noindent\textbf{Author contribution} TODO.
