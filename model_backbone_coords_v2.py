...
class DoublePointDetectionHead(nn.Module):
    def __init__(
        ...
        guidance_enabled: bool = False,      # v2: отключено по умолчанию
        ...
        # v2: улучшенный Offset Head — более глубокий для sub-pixel точности
        ...
        # boundary_weights оставляем как заглушку для совместимости формата выхода
    ...
    def forward(self, x, guidance_weight: float = 0.0):
        ...
        # v2: boundary guidance убрана — не используем bnd для модификации фичей
        bnd = self.boundary_weights(f)  # заглушка для совместимости
        f_guided = f
...
class LitBoundaryAwareSystem(pl.LightningModule):
    ...
    def shared_step(self, batch, stage: str):
        ...
        # v2: boundary loss и guidance полностью отключены
        ...
        # ----- boundary-loss: v2 — полностью отключён -----
