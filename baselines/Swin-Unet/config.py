...
try:
    ...
    class CN(dict):
        """Minimal yacs-compatible fallback used when yacs is unavailable."""
        ...
        @staticmethod
        def _convert(value):
            if isinstance(value, dict) and not isinstance(value, CN):
                return CN(value)
            if isinstance(value, list):
                return [CN._convert(v) for v in value]
            return value
        ...
        def _merge_dict(self, src):
            for k, v in src.items():
                if isinstance(v, dict):
                    if k not in self or not isinstance(self[k], CN):
                        self[k] = CN()
        ...
        def merge_from_list(self, cfg_list):
            if len(cfg_list) % 2 != 0:
                raise ValueError("opts must be in KEY VALUE pairs")
            for full_key, raw_value in zip(cfg_list[0::2], cfg_list[1::2]):
                node = self
                key_parts = full_key.split(".")
                for key in key_parts[:-1]:
                    if key not in node or not isinstance(node[key], CN):
                        node[key] = CN()
                    node = node[key]
                try:
